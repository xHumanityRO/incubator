...
<#macro moderationImages>
	...
	function deleteTopic() {
		if (confirm("${I18n.getMessage("Moderation.ConfirmDelete")}")) {
			...
			document.formModeration.log_description.value = 'deleting thread';
	...
	function lockUnlock(lock) {
		...
		if (lock) {
			document.formModeration.log_description.value = 'locking thread';
			todo("topicLock");
		} else {
			document.formModeration.log_description.value = 'unlocking thread';
			todo("topicUnlock");
		}
		document.formModeration.submit();
	}
